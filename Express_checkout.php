...
class Express_checkout extends CI_Controller
{
	...
	function SetExpressCheckout()
	{
		...
		$SECFields = array(
			...
			'hdrimg' => base_url('static/img/EC_logo-Black.png'), // URL for the image displayed as the header during checkout.  Max size of 750x90.  Should be stored on an https:// server or you'll get a warning message in the browser.
			'logoimg' => base_url('static/img/EC_logo-Black.png'), // A URL to your logo image.  Formats:  .gif, .jpg, .png.  190x60.  PayPal places your logo image at the top of the cart review area.  This logo needs to be stored on a https:// server.
			'brandname' => 'Abdul Khan', // A label that overrides the business name in the PayPal account on the PayPal hosted checkout pages.  127 char max.
	...
	function DoExpressCheckoutPayment()
	{
		...
		$Payment = array(
			'amt' => $cart['shopping_cart']['grand_total'], 	    // Required.  The total cost of the transaction to the customer.  If shipping cost and tax charges are known, include them in this value.  If not, this value should be the current sub-total of the order.
			'itemamt' => $cart['shopping_cart']['subtotal'],       // Subtotal of items only.
	...
	function OrderComplete()
	{
		...
		$user = $this->accounts_model->getAccountById($this->session->myId);

		$this->load->model('products_model');
		$this->products_model->add_payment_plan($cart, $user, 'paid');

		//Product store update
		$this->products_model->update_stock($cart);
	...
	function OrderCancelled()
	{
		...
		$this->load->model('products_model');
		$this->products_model->add_payment_plan(NULL, $user, 'unpaid');
