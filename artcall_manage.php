				...
				<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
					...
					<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
						<header style="background: #4c4f53; color: white; height: 50px; margin-left: -15px; padding: 16px">
							<span style="font-size: 10pt;">ArtCall Manage</span>
						</header>
						...
						<div class="jarviswidget jarviswidget-color-blueDark" id="wid-id-1" data-widget-editbutton="false" style="margin-bottom: 50px">
							<form action="<?= base_url()?>starter/ordreslt" id="login-form" class="smart-form client-form" method="post">
								<header>
									<div class="col-sm-12">
										<?php if(isset($this->session->artcall_id) && $this->session->action=='update'){?>
										<div class="alert alert-success fade in" id="suss">
											<button class="close" data-dismiss="alert">
												×
											</button>
											<i class="fa-fw fa fa-check"></i>
											<strong>Success! </strong> artcall_id #<?= $this->session->artcall_id ?> Info updated.
										</div>
										<?php }elseif(isset($this->session->artcall_id) && $this->session->action!='update'){?>
										<div class="alert alert-danger fade in" id="err">
											<button class="close" data-dismiss="alert">
												×
											</button>
											<i class="fa-fw fa fa-times"></i>
											<strong>Deleted!</strong> artcall_id #<?= $this->session->artcall_id ?> was removed.
										</div>
										<?php } ?>
									</div>
								</header>
									...
									<table id="datatable_fixed_column" class="table table-striped table-bordered" width="100%">
								        ...
								        <thead>
								            <tr>
							                    ...
							                    <th data-hide="phone,tablet">Awards</th>
							                    <th >Suggester</th>							                    
										...
										<?php foreach($artcalls as $key => $row){?>
											<tr>												
												...
												<td><img src="<?= base_url('upload/').$row['img']?>" width="50"></td>
												...
												<td align="center">
										        	...
										        	<label class="btn bg-color-teal txt-color-white"
										        		title="Edit Row" data-toggle="modal" data-target="#myModal_" onclick="editArtCall(this, <?= $row['id']?>, 'edit')">
														<span class="fa fa-edit"></span>
													</label>
	    ...
	    <div class="modal-content">
	        ...
	        <div class="modal-body">
	        	<div class="row">
	        		...
	        		<div class="col-sm-4"><label for="usr">Description:</label><input type="text" class="form-control" id="description" pattern="[A-Za-z0-9]+"></div>
				...
				<div class="row" style="padding-top: 5px">
	        		...
	        		<div class="col-sm-4"><label for="usr">Awards:</label><input type="text" class="form-control" id="awards" pattern="[A-Za-z0-9]+"></div>
				</div>	
				<input type="hidden" id="Rid">
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-primary" data-dismiss="modal" onclick="updateArtCall()">&nbsp;Save&nbsp;</button>
...
<script type="text/javascript">
	...
	function updateArtCall(){
		...
		window.location.href='<?= base_url('admin/artcallmanproc')?>?act=update&params='+JSON.stringify(params);
	}

	function delScript(id){
		let res = window.confirm('Are you sure to delete?');
		if(res)
			window.location.href='<?= base_url('admin/artcallmanproc')?>?rqid='+id+'&act=del';
	}
</script>
