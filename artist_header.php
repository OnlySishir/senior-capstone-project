...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
  ...
  <div class="collapse navbar-collapse" id="navbarsExample03">
    <form class="form-inline my-2 my-md-0" action="<?php echo site_url('starter/searchResult');?>" method = "post">
    ...
    <ul class="navbar-nav ml-auto">
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="dropdown01">    
         <a class="dropdown-item" href="<?= base_url('product/shop')?>">All Art & Design</a>
          <?php foreach($categroysorts as $key => $row){?>
            <?php 
              $act_class = "";
              if(!empty($_GET['act']))
                $act_class = $_GET['act']==$row['category_sort'] ? "active" : "";
            ?>
            <a class="dropdown-item <?= $act_class?>" href="<?= base_url('product/shop/').$row['id'].'?act='.$row['category_sort']?>"><?= $row['category_sort']?></a>
          <?php }?>          
        </div>
      </li>&nbsp;&nbsp;&nbsp;
      <!--<li class="nav-item">-->
      ...
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
          ...
          <a class="dropdown-item" href="<?= base_url('addproduct')?>">Product Listing</a>
          <a class="dropdown-item" href="#">Connections</a>
        </div>
      </li>

      <li class="nav-item dropdown">
				<a class="nav-link dropdown-toggle" id="navbarDropdownMenuLink-4" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="cursor: pointer;">
          ...
          <a class="dropdown-item" href="<?= base_url('profile').'?email='.$this->session->myEmail?>">My Account</a>
