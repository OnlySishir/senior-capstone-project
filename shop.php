        ...
        <div class="col-md-3 col-sm-6">
            <div class="product-grid2">
                <div class="product-image2">
                    ...
                    <ul class="social">
                        <li><a href="#" data-tip="Quick View"><i class="fa fa-eye" style="margin-top: 15px"></i></a></li>
                        <li>
                            <?php $price = $this->session->certified=='ok' ? $row["pro_wholesale"] : $row["pro_retail"]; ?>
                            <a href="javascript:add_cart_func_(<?= $row['id']?>, '<?= $row["pro_title"]?>', 1)" data-tip="Add to Cart">
                                <i class="fa fa-shopping-cart" style="margin-top: 15px"></i>
                            </a>
                        </li>
                    </ul>
                    <a class="add-to-cart" href="javascript:add_cart_func_(<?= $row['id']?>, '<?= $row["pro_title"]?>', 1)" >Add to cart</a>
                    <input class="price" type="hidden" name="real-price<?= $row['id']?>" value="<?= $row["pro_retail"]?>">
                </div>
                    ...
                    <?php if( $this->session->certified == 'ok' ){?>
                        <div class="row">
                            <div class="col-sm-6 radio">
                                <label>
                                    <input type="radio" name="optradio<?= $row['id']?>" checked value="<?= $row['pro_retail']?>"> <span class= "price"> $ <?= $row['pro_retail']?></span>
                                </label>
                            </div>
                            <?php if (($row['pro_wholesale'] != NULL) and ($row['pro_wholesale'] != 0))  { ?>
                            <div class="col-sm-6 radio">
                                <label>
                                    <input type="radio" name="optradio<?= $row['id']?>" value="<?= $row['pro_wholesale']?>"> <span class= "price"> $ <?= $row['pro_wholesale']?></span>
                                </label>
                            </div>   
                            <?php } else { ?>
                            <?php } ?>                         
