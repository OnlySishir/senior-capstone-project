				...
				<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
					...
					<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
						<header style="background: #4c4f53; color: white; height: 50px; margin-left: -15px; padding: 16px">
							<span style="font-size: 10pt;">Users Manage</span>
						</header>
						...
						<div class="jarviswidget jarviswidget-color-blueDark" id="wid-id-1" data-widget-editbutton="false" style="margin-bottom: 50px">
							<form action="<?= base_url()?>starter/ordreslt" id="login-form" class="smart-form client-form" method="post">
								<header>
									<div class="col-sm-12">
										<?php if(isset($this->session->action) && $this->session->action=='update'){?>
										<div class="alert alert-success fade in" id="suss">
											<button class="close" data-dismiss="alert">
												×
											</button>
											<i class="fa-fw fa fa-check"></i>
											<strong>Success! </strong> Account id #<?= $this->session->account_id ?> Info updated.
										</div>
										<?php }elseif(isset($this->session->account_id) && $this->session->action!='update'){?>
										<div class="alert alert-danger fade in" id="err">
											<button class="close" data-dismiss="alert">
												×
											</button>
											<i class="fa-fw fa fa-times"></i>
											<strong>Deleted!</strong> Account id #<?= $this->session->account_id ?> was removed.
										</div>
										<?php } ?>
									</div>
								</header>
									...
									<table id="datatable_fixed_column" class="table table-striped table-bordered" width="100%">
								        ...
								        <thead>
								            <tr>
							                    ...
							                    <th data-hide="phone,tablet">Info</th>
							                    <th >Roles</th>							                    
										...
										<tbody>											
											<!-- <tr>												
												...
												<td>110220</td>
												<td>2018-03-03</td>
												<td>google.com/about</td>
												<td>Friend Introduce</td>
												<td align="center">
													<a href="#" title="Edit Row" data-toggle="modal" data-target="#myModal_">
														<span class="fa fa-edit"></span>
													</a>
													<a href="javascript:void(0)" title="Delete selected row" onclick="window.confirm('Are you sure?')">
	    ...
	    <div class="modal-dialog">
		    <div class="modal-content">
		        <div class="modal-header">
		          ...
		          <h4 class="modal-title" style="font-weight: bold;">Update User Info</h4>
		        </div>
		        <div class="modal-body">
		        	<div class="row">
		        		<div class="col-sm-4"><label for="usr">FirstName:</label><input type="text" class="form-control" id="fName"></div>
		        		<div class="col-sm-4"><label for="usr">LastName:</label><input type="text" class="form-control" id="lName"></div>
		        		<div class="col-sm-4"><label for="usr">Email:</label><input type="email" class="form-control" id="email"></div>
					</div>	
					<div class="row" style="padding-top: 5px">
		        		<div class="col-sm-4"><label for="usr">Zip Code:</label><input type="text" class="form-control" id="postcode"></div>
		        		<div class="col-sm-4"><label for="usr">Link:</label><input type="text" class="form-control" id="link"></div>
		        		<div class="col-sm-4"><label for="usr">Roles:</label><input type="text" class="form-control" id="role"></div>
					</div>	
					<input type="hidden" id="Rid">
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-primary" data-dismiss="modal" onclick="UpdateAccount()">&nbsp;Save&nbsp;</button>
...
<script type="text/javascript">
	...
	//modal update
	function editAccount(e, id, act){
		let dom = $(e).parent().parent().children();
		$('#fName').val($(dom[0]).text().trim());
		$('#lName').val($(dom[1]).text().trim());
		$('#email').val($(dom[2]).text().trim());
		$('#postcode').val($(dom[3]).text().trim());
		$('#link').val($(dom[5]).text().trim());
		$('#role').val($(dom[8]).text().trim());
		$('#Rid').val(id);
	}
	...
	function UpdateAccount(){
		...
		window.location.href='<?= base_url('admin/usermanproc')?>?act=update&params='+JSON.stringify(params);
	}

	function delScript(id){
		let res = window.confirm('Are you sure to delete?');
		if(res)
			window.location.href='<?= base_url('admin/usermanproc')?>?rqid='+id+'&act=del';
	}
</script>
